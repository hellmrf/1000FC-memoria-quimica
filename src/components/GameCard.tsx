...
type GameCardProps = typeof Card & {
  cardSize: number;
  borderWidth?: number;
  CardImage?: React.FunctionComponent<React.SVGAttributes<SVGElement>>;
};
export default (props: GameCardProps) => {
  const CardImage = props.CardImage || Logotipo;
  return (
    <Card activeOpacity={0.5} {...props}>
      <CardImage width={props.cardSize || CARD_SIZE * 0.75} fill="black" />
    </Card>
  );
};
